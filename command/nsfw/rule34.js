...
exports.run = async (client, msg, args) => {
    ...
    booru.search('rule34', [`${args.join('_')}`,'rating:explicit'], {limit:1,random:true}).then(async results => {
        let posts = results.posts;
        for (let post of posts) {
            if (post.sample_url.endsWith('.mp4')) return msg.reply('````Hentai - ' + args.join(' ') + '```\n' + post.sample_url);
...
exports.info = {
    description: "Search some hentai pics on Rule34",
    aliases: ["r34"]
};